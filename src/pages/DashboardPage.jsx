...
function EnquiriesSection() {
  ...
  const leads = data?.myEnquiries?.items || [];
            ...
            <div className="sh-item" key={l.id} style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div className="sh-meta">{l.property?.city} · {l.assignedAgent?.name || 'Agent'} · {new Date(l.createdAt).toLocaleDateString('en-IN')}</div>
                </div>
    ...
    <div>
      ...
      {reviews.length === 0
        ? <EmptyState
          ...
          description="After visiting a property, leave a review to help other buyers."
        />
        : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {reviews.map(r => (
                    ...
                    <div style={{ display: 'flex', gap: 2 }}>
                      {[1, 2, 3, 4, 5].map(i => (
                        <i
                          ...
                          style={{
                            fontSize: 14,
                            color: i <= r.rating ? 'var(--gold)' : 'var(--border2)'
                          }}
                        />
                      ))}
                    </div>

                    <span className={`badge ${RBADGE[r.status] || 'badge-navy'}`}>
                      {r.status}
                    </span>
                  </div>
                </div>
