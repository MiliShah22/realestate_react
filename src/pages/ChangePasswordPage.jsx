...
function pwdStrength(pwd) {
  ...
  return {
    ...
    color: ['','#e84040','#f0a020','#2d8a56','#16a34a'][s],
    width: `${s * 25}%`,
  };
}
...
export default function ChangePasswordPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (currentPwd === newPwd)                  { setError('New password must differ from current password.'); return; }
          ...
          <div className="post-form" style={{background:'#fff',border:'0.5px solid var(--border)',borderRadius:'var(--radius)',padding:32}}>
            ...
            {!success ? (
              <>
                ...
                {error && (
                  <div className="form-error"><i className="ti ti-alert-circle"></i> {error}</div>
                )}
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="form-group">
                    ...
                    <div className="input-icon-wrap">
                      <i className="ti ti-lock"></i>
                      <input className="form-control" type={showCurrent?'text':'password'}
                    ...
                    <div style={{textAlign:'right',marginTop:4}}>
                      <Link to="/forgot-password" style={{fontSize:12,color:'var(--gold2)',fontWeight:500}}>
                  ...
                  <div className="form-group">
                    ...
                    <div className="input-icon-wrap">
                      ...
                      <input className="form-control" type={showNew?'text':'password'}
                        placeholder="Create new password" value={newPwd}
                    ...
                    {newPwd && (
                      <>
                        ...
                        <div className="strength-label" style={{color:pwd.color}}>Password strength: {pwd.label}</div>
                        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,marginTop:8}}>
                          {CHECKS.map(([label, fn]) => (
                            <div key={label} style={{fontSize:11,color:fn(newPwd)?'var(--success)':'var(--text3)',display:'flex',alignItems:'center',gap:4}}>
                              <i className={`ti ${fn(newPwd)?'ti-check':'ti-x'}`} style={{fontSize:12}}></i> {label}
                            </div>
                          ))}
                        </div>
                      </>
                  ...
                  <div style={{display:'flex',gap:10,marginTop:8}}>
                    <button type="button" className="btn-outline" style={{flex:1,padding:12}} onClick={() => navigate('/dashboard')}>
                      Cancel
                    </button>
                    <button type="submit" className="btn-full btn-full-navy" style={{flex:2}} disabled={loading}>
                      {loading
                        ? <><i className="ti ti-loader-2" style={{animation:'spin 1s linear infinite'}}></i> Updating…</>
